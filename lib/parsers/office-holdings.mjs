...
// Map each SKU to the OC order number(s) that contain it. If the same SKU
// appears in several OCs, list them joined by " + " (matches the
// "OC 1 + OC 2" style used in the discrepancy sheet header).
export function ocsBySku(ocs) {
  const m = new Map();
  for (const oc of ocs) {
    for (const sku of oc.items.keys()) {
      const prev = m.get(sku);
      if (!prev) m.set(sku, [oc.orderNumber]);
      else if (!prev.includes(oc.orderNumber)) prev.push(oc.orderNumber);
    }
  }
  const out = new Map();
  for (const [sku, list] of m) out.set(sku, list.join(' + '));
  return out;
}
...
export function build(parsed) {
  ...
  const ocBySku = parsed.ocBySku; // Map<sku,string> | undefined
  const ocOf = (sku) => (ocBySku && ocBySku.get(sku)) || '';
