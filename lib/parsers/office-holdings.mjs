...
export async function parsePoPdf(buffer, fallbackLabel) {
  ...
  if (!sizeCols) {
    // Aloha OCs accidentally uploaded into the PO field have a very distinct
    // shape ([STYLE-VVSS] SKU anchors with a parenthesised size). Surface
    // that specifically so the user knows what to fix.
    const flatTokens = allRows.flat();
    const ocBracketHits = flatTokens.filter((q) =>
      /^\[[A-Z0-9]+-[A-Z0-9]+\].*\(\d{2}\)/.test(q.s)
    ).length;
    if (ocBracketHits >= 3) {
      warnings.push(
        `Este PDF parece ser una Order Confirmation (Aloha), no un PO de Office Holdings ` +
          `(detectados ${ocBracketHits} SKUs con formato "[XXXX-YYYY] (SS)"). ` +
          `Súbelo en el campo "OC — Order Confirmation", no en el de POs.`
      );
      return { poNumber: poNumber ?? fallbackLabel ?? 'PO', items, totalQty, warnings };
    }
...
// Greedy per-PO allocation of OC units. For each PO (in upload order) and
// each SKU, draw the needed quantity from the OCs (also in upload order),
// draining each OC's remaining stock before moving to the next. Returns an
// array indexed by PO position: result[poIdx] is Map<sku, "OC123"> when a
// single OC covers the row, or Map<sku, "OC123 + OC124"> when the row
// straddles two (or more) OCs because an OC ran out mid-PO.
//
// Why greedy in upload order: it gives a single OC per PO whenever possible
// (the user's preference) and is fully deterministic — re-uploading OCs in a
// different order is how the user steers the assignment.
export function allocateOcsToPos(ocs, pos) {
  const ocRemaining = ocs.map((oc) => {
    const items = new Map();
    for (const [sku, qty] of oc.items) items.set(sku, qty);
    return { orderNumber: oc.orderNumber, items };
  });
  return pos.map((po) => {
    const agg = new Map();
    for (const it of po.items) agg.set(it.sku, (agg.get(it.sku) ?? 0) + it.qty);
    const row = new Map();
    for (const [sku, qtyNeeded] of agg) {
      let remaining = qtyNeeded;
      const used = [];
      for (const oc of ocRemaining) {
        if (remaining <= 0) break;
        const avail = oc.items.get(sku) ?? 0;
        if (avail <= 0) continue;
        const take = Math.min(remaining, avail);
        oc.items.set(sku, avail - take);
        if (!used.includes(oc.orderNumber)) used.push(oc.orderNumber);
        remaining -= take;
      }
      if (used.length) row.set(sku, used.join(' + '));
    }
    return row;
  });
}
...
export function reconcile(ocs, pos) {
  ...
  for (const sku of [...skus].sort()) {
    ...
    if (!inAnyPo) {
      rank = 2;
      motivo = `Solo en OC Alohas, ningún PO Office Holdings lo trae (faltan ${ocTotal} en PO)`;
    } else if (ocTotal === 0) {
      rank = 3;
      motivo = 'En PO Office Holdings, sin OC Alohas';
    } else {
      rank = 1;
      motivo =
        dif > 0
          ? `Office Holdings pide ${dif} más que Alohas confirma (sobran en PO)`
          : `Alohas confirma ${-dif} más de lo que Office Holdings pide (faltan en PO)`;
    }
  ...
  return {
    ...
    title: 'DISCREPANCIAS — POs Office Holdings vs OC Alohas',
    legend:
      'Diferencia = (suma POs Office Holdings) − (OC Alohas).  ' +
      'Positiva: Office Holdings pide MÁS de lo que Alohas confirma (sobran en PO).  ' +
      'Negativa: Alohas confirma MÁS de lo que Office Holdings pide (faltan en PO). ' +
      'Si la diferencia es negativa con todas las POs en positivo, es porque la OC de Alohas es mayor que la suma de las POs para ese SKU.',
    summaryLabels: {
      real: 'Reales Office Holdings ≠ Alohas',
      refOnly: 'Solo en OC Alohas',
      colOnly: 'Solo en PO Office Holdings',
    },
  };
}
...
export function build(parsed) {
  ...
  const ocByPo = parsed.ocByPo; // Map<sku,string>[] indexed by PO | undefined
  const ocOf = (poIdx, sku) => (ocByPo && ocByPo[poIdx] && ocByPo[poIdx].get(sku)) || '';
  ...
  parsed.pos.forEach((po, idx) => {
    const aoa = [];
    aoa.push(['SKU', 'QUANTITY', 'PRICE', 'DISCOUNT', 'OC', null, 'PO:', po.poNumber]);
    ...
    if (entries.length === 0 && warningText) {
      aoa.push([null, null, null, null, null, null, 'Aviso:', warningText]);
    }
    entries.forEach(([sku, qty], i) => {
      const [price, disc] = priceOf(sku);
      const oc = ocOf(idx, sku);
      if (i === 0 && warningText) {
        aoa.push([sku, qty, price, disc, oc, null, 'Aviso:', warningText]);
      } else {
        aoa.push([sku, qty, price, disc, oc]);
      }
    });
