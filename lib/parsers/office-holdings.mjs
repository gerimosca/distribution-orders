...
// exactly 6 size columns. Strategy: take all pure integers to the right
// of the factory-style token (prices/discounts are excluded because they
// carry decimals or %); if the last one is far past the rest, treat it as
// the row total and drop it. Standard EU 36..41 mapping then applies.
function inferSizeColsFromItemRow(parts, fsX) {
  const ints = parts
    .filter((q) => q.x > fsX && /^\d+$/.test(q.s))
    .sort((a, b) => a.x - b.x);
  if (ints.length < DEFAULT_EU_RUN.length) return null;
  // Drop a trailing row-total: if there are 7 integers and the gap from
  // the last to the previous is >3x the average inner gap, the last one
  // is the row total.
  if (ints.length === DEFAULT_EU_RUN.length + 1) {
...
export async function parsePoPdf(buffer, fallbackLabel) {
  ...
  if (!sizeCols) {
    for (const parts of allRows) {
      const fs = parts.find((q) => FACTORY_STYLE.test(q.s));
      if (!fs) continue;
      const inferred = inferSizeColsFromItemRow(parts, fs.x);
      if (inferred) {
        sizeCols = inferred;
  ...
  if (!sizeCols) {
    // Capture a small diagnostic so we know what pdfjs actually returned
    // on the runtime that's failing (positions or token shapes may differ
    // between the local and serverless runtimes).
    const fsCandidates = [];
    const intCounts = [];
    for (const parts of allRows) {
      const fs = parts.find((q) => FACTORY_STYLE.test(q.s));
      if (!fs) continue;
      fsCandidates.push(`${fs.s}@x=${Math.round(fs.x)}`);
      const ints = parts
        .filter((q) => q.x > fs.x && /^\d+$/.test(q.s))
        .map((q) => `${q.s}@${Math.round(q.x)}`);
      intCounts.push(ints.length ? ints.join(',') : '∅');
    }
    const diag = fsCandidates.length
      ? `factory styles vistos=${fsCandidates.length} (${fsCandidates.slice(0, 3).join(' / ')}); ints por fila=[${intCounts.slice(0, 3).join(' | ')}]`
      : 'ningún factory style detectado en el PDF';
    warnings.push(
