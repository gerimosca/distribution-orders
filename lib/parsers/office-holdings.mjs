...
export async function parsePoPdf(buffer, fallbackLabel) {
  ...
  if (!sizeCols) {
    // Rich diagnostic for the next iteration if this still fails on the
    // serverless runtime: factory styles seen, plus a sample of tokens
    // (with x positions) from the first item-anchor rows OR — if none —
    // from the first rows in the document so we can see what pdfjs is
    // actually emitting.
    let diag;
    if (itemAnchors.length) {
      ...
      const sample = allRows
        ...
        .join(' || ');
      diag = `ningún factory style detectado. Muestra de tokens: ${sample}`;
    }
    warnings.push(
