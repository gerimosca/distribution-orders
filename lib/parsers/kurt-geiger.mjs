...
export function parseKurtGeiger(buffer) {
  ...
  for (const b of blocks) {
    // items as Map<sku, qty> so the same SKU appearing in multiple rows for
    // the same warehouse stays as one entry. Converted to array on output.
    warehouseData.set(b.name, { items: new Map(), pos: new Set(), warnings: [] });
  }
  ...
  for (let r = L.dataStartIdx; r < rows.length; r++) {
    ...
    for (let bi = 0; bi < blocks.length; bi++) {
      ...
      for (let i = 0; i < sizes.length; i++) {
        ...
        const wh = warehouseData.get(b.name);
        wh.items.set(sku, (wh.items.get(sku) ?? 0) + qty);
      }
...
// Excel caps tab names at 31 chars; tabNameFor's slice can collide if two
// unknown warehouses share their first 31 characters. Append "(2)", "(3)"...
// reserving space for the suffix.
function uniqueTabName(used, base) {
  if (!used.has(base)) {
    used.add(base);
    return base;
  }
...
export function buildWorkbook(parsed) {
  ...
  if (parsed.discrepancies) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, uniqueTabName(usedTabs, 'Discrepancias'));
  }
  ...
  // File-level issues (row totals not adding up etc.) only have one natural
  // home — prepend them to the first warehouse tab so the user sees them.
  const globalIssues = parsed.globalIssues ?? [];

  parsed.warehouses.forEach((wh, idx) => {
    const aoa = [];
    ...
    const warningText = warningParts.length
      ? 'Revisar: ' + warningParts.join(' | ')
      : null;
    if (wh.items.length === 0 && warningText) {
      aoa.push([null, null, null, null, null, 'Aviso:', warningText]);
    }
    wh.items.forEach((it, i) => {
      const [price, disc] = priceOf(it.sku);
      if (i === 0 && warningText) {
        aoa.push([it.sku, it.qty, price, disc, null, 'Aviso:', warningText]);
      } else {
        aoa.push([it.sku, it.qty, price, disc]);
      }
    });
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    XLSX.utils.book_append_sheet(wb, ws, uniqueTabName(usedTabs, tabNameFor(wh.warehouse)));
  });
  return wb;
}
